...
function CanvasToolbar() {
  ...
  return (
    <div className="mb-3 flex items-center gap-1 px-1">
      <button type="button" onClick={() => { try { actions.history.undo(); } catch { /* noop */ } }}
        className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-100">
        ↩ 撤銷
      </button>
      <button type="button" onClick={() => { try { actions.history.redo(); } catch { /* noop */ } }}
        className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-zinc-600 hover:bg-zinc-100">
        ↪ 重做
      </button>
      {selectedName && (
...
function EditorCanvas({
  ...
  const hasData = contentJson && contentJson !== "{}";

  return (
    <Frame data={hasData ? contentJson : undefined}>
      <Element is={CanvasContainer} canvas>
        <Element is={Container} padding={24} canvas>
          <TextComponent text="在這裡開始編輯…" fontSize={16} color="#6e6e73" weight="normal" />
        </Element>
      </Element>
    </Frame>
  );
...
export default function EditPostPage() {
  ...
  useEffect(() => {
    if (!selectedTemplateId && templates.length > 0) {
      const def = templates.find((t) => t.name === "default") ?? templates[0];
  ...
  const handlePreview = () => {
    ...
    localStorage.setItem("lumina_temp_preview", JSON.stringify({ html: fullHtml, title, postId: id }));
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "更新時發生錯誤");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (notFound) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#F5F5F7]">
        <p className="text-lg font-semibold text-zinc-700">找不到此文章</p>
            ...
            <div className="mx-auto w-full max-w-7xl">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">編輯文章</h1>
                    <p className="text-sm text-zinc-600">修改後點擊「更新文章」即可儲存。</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {draftSaved && (
                  ...
                  <button type="button" onClick={handleUpdate} disabled={saving || drafting}
                    className="rounded-full bg-black px-6 py-2 text-sm font-medium text-white shadow-md hover:bg-zinc-800 disabled:opacity-60">
                    {saving ? "更新中…" : "更新文章"}
                  </button>
                </div>
              </div>
            ...
            <div className="hidden w-56 shrink-0 lg:block">
              <div className={`sticky ${sidebarTop} flex flex-col gap-4`}>
                <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5">
                  ...
                  <input
                    className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm text-zinc-900 outline-none focus:border-zinc-800 focus:ring-1 focus:ring-zinc-800"
                    value={title}
              ...
              <div className="rounded-3xl bg-white/80 p-4 shadow-sm ring-1 ring-white/60 backdrop-blur-xl">
                ...
                <div className="flex justify-center">
                  <div className="w-full max-w-[800px] rounded-[24px] bg-white p-6 shadow-sm ring-1 ring-black/5">
                    <EditorCanvas contentJson={contentJson} queryRef={serializeRef} />
                  </div>
                </div>
              </div>
            </div>
